...
void *slab_alloc(struct slab_chain *const sch)
{
    ...
    if (LIKELY(sch->partial != NULL)) {
        ...
        int pages_num = (sch->slabsize + slab_pagesize -1 )/slab_pagesize;
        sch->partial = (struct slab_header *)po_extend(sch->pod, pages_num * slab_pagesize, PROT_READ|PROT_WRITE, MAP_PRIVATE);
        if (sch->partial == NULL) {
            printf("malloc from po failed");
            return NULL;
        }
        //printf("po_extend\n");
...
void slab_free(struct slab_chain *const sch, const void *const addr)
{
    ...
    struct slab_header *const slab = (struct slab_header *)((uintptr_t) addr & sch->alignment_mask);
    ...
    if (UNLIKELY(slab->slots == SLOTS_ALL_ZERO)) {
        ...
        if (UNLIKELY(slab->refcount == 1 || (slab->refcount == 0 &&
            ...
            for (; s.c != page_end; s.c += sch->slabsize) {
            ...
            if (po_shrink(sch->pod, (unsigned long)page, slab_pagesize) == -1) {
                printf("po_shrink failed\n");
            }
            //printf("po_shrink\n");
